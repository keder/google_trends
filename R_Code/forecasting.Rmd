...
my_scale <- function(data) {
    if (any(is.na(data)) || all(data == 0, na.rm = TRUE)) {
        warning("Zero or NA sequence scaling")
...
create_pca_features <- function(df) {
    df <- df[, colSums(is.na(df)) == 0]
    predictors <- df[, -c(1, 2)]
    date_and_rate <- df[, c(1, 2)]
    pca_features <- prcomp(predictors, center = TRUE)

    explained_var <- summary(pca_features)$importance[3, ]
    num_components <- which(cumsum(explained_var) >= 0.9)[1]

    pca_features <- as.data.frame(pca_features$x[, 1:num_components])
    colnames(pca_features) <- paste0("pc", 1:num_components)

    result <- cbind(date_and_rate, pca_features)
...
.predict_arfima <- function(obj, df) {
    forecast_length <- nrow(df)
    # print(xreg)
    if (ncol(df) > 1) {
        xreg <- as.matrix(df[, -1])
...
pca_prediction_pipe <- function(df, method, fit_lengths, horizons, transformation) {
    # We shift values by +1. This required for some models to converge.
    shift_const <- 1
    selected_regressors <- names(df[, -c(1, 2, 3)])
    nested_data <- df %>%
        select(c("ds", "state", "rate", selected_regressors)) %>%
        group_by(state) %>%
        mutate(across(-1, my_scale)) %>%
        ...
        mutate(across(-c(1, 2), ~ . + shift_const)) %>%
        nest_by(state) %>%
        mutate(data = list(create_pca_features(data))) %>%
        split(seq(nrow(.)))
    column_counts <- map_dfr(nested_data, function(row) {
        tibble(
            state = row$state[[1]], # extract scalar from tibble column
            n_cols = ncol(row$data[[1]]) - 2 # extract the actual nested df
        )
    })
    print(paste0(
        "Principal component number ", 
        mean(column_counts$n_cols), 
        " (", 
        min(column_counts$n_cols),
        ", ",
        max(column_counts$n_cols),
        ")"))
    # mutate(prediction = list(my_better_rollapply(
    #     data,
    #     roll_fun,
...
run_model <- function(df, method, fit_lengths, horizons, transformation, top_trends, bottom_trends) {
    ...
    pca_result <- pca_prediction_pipe(
        df,
        method,
        fit_lengths,
        horizons,
    ...
    pca_result$model <- paste0(method, "(rate, principal components)")
    result <- rbind(
        result,
        pca_result
    )
...
incidence_arfima_predictions_file <- paste0(output_dir, "/incidence_arfima_predictions.RData")
mortality_arfima_predictions_file <- paste0(output_dir, "/mortality_arfima_predictions.RData")
...
```{r}
incidence_arfima_predictions <- run_model(
    incidence_plus_selected_trends,
    "arfima",
    fit_lengths,
    horizons,
...
mortality_arfima_predictions <- run_model(
    mortality_plus_selected_trends,
    "arfima",
    fit_lengths,
    horizons,
...
save(incidence_arfima_predictions, file = incidence_arfima_predictions_file)
save(mortality_arfima_predictions, file = mortality_arfima_predictions_file)
```
...
# load(file = incidence_arfima_predictions_file)
# load(file = mortality_arfima_predictions_file)
...
# incidence_arfima_predictions$model_family <- "arfima"
...
# mortality_arfima_predictions$model_family <- "arfima"
...
mortality_absolute_improvement <- mortality_metrics %>%
    ...
    select(-order_index) %>%
    mutate(model = str_replace_all(model, "arima", "ARIMA")) %>%
    mutate(model = str_replace_all(model, "arima", "ARFIMA")) %>%
    mutate(model = str_replace_all(model, "prophet", "Prophet")) %>%
    mutate(model = str_replace_all(model, "xgboost", "XGBoost")) %>%
    as.data.frame()
```
